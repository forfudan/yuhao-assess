...
import type { 單字頻簡碼效率結果介面 } from '../atoms/shortCodeEfficiency'
...
const 計算單字頻簡碼效率 = (
  ...
  // 計算不同 N 值下的效率
  const N值列表 = [
    0, 5, 10, 25, 50, 75, 100, 150, 200, 250, 300, 400, 500, 600, 700, 800, 900, 1000, 1100, 1200,
    1300, 1400, 1500, 1600, 1700, 1800, 1900, 2000,
  ]
  const 結果: 單字頻簡碼效率結果介面[] = []

  for (const N of N值列表) {
    // 只考慮簡碼長度小於全碼長度且小於最大碼長的漢字
    const 有效簡碼字符 = 處理後字符.filter(c => c.簡碼長度 < c.全碼長度 && c.簡碼長度 < 最大碼長)

    // 按字頻碼長乘積排序，選擇前 N 個字符使用簡碼
...
const ShortCodeEfficiencyPage: React.FC = () => {
  ...
  const 計算簡碼效率 = async () => {
    ...
    try {
      ...
      const 新結果 = {
        知乎簡體字頻結果: 計算單字頻簡碼效率(知乎簡體字頻, 簡碼映射, 全碼映射, 最大碼長),
        北語簡體字頻結果: 計算單字頻簡碼效率(北語簡體字頻, 簡碼映射, 全碼映射, 最大碼長),
        臺標繁體字頻結果: 計算單字頻簡碼效率(臺標繁體字頻, 簡碼映射, 全碼映射, 最大碼長),
        古籍繁體字頻結果: 計算單字頻簡碼效率(古籍繁體字頻, 簡碼映射, 全碼映射, 最大碼長),
        繁簡聯合字頻結果: 計算單字頻簡碼效率(繁簡聯合字頻, 簡碼映射, 全碼映射, 最大碼長),
        計算時間: Date.now(),
      }
  ...
  const 表格數據 = useMemo<表格行數據[]>(() => {
    ...
    // 獲取所有 N 值
    const N值列表 = 知乎簡體字頻結果.map(r => r.最有效率的簡碼個數)

    const 所有行 = N值列表.map(N => {
      const 知乎結果 = 知乎簡體字頻結果.find(r => r.最有效率的簡碼個數 === N)
      const 北語結果 = 北語簡體字頻結果.find(r => r.最有效率的簡碼個數 === N)
      const 臺標結果 = 臺標繁體字頻結果.find(r => r.最有效率的簡碼個數 === N)
      const 古籍結果 = 古籍繁體字頻結果.find(r => r.最有效率的簡碼個數 === N)
      const 繁簡結果 = 繁簡聯合字頻結果.find(r => r.最有效率的簡碼個數 === N)
      ...
      return {
        ...
        知乎簡體字頻字符: 知乎結果?.對應字符列表 || [],
        北語簡體字頻字符: 北語結果?.對應字符列表 || [],
        臺標繁體字頻字符: 臺標結果?.對應字符列表 || [],
        古籍繁體字頻字符: 古籍結果?.對應字符列表 || [],
        繁簡聯合字頻字符: 繁簡結果?.對應字符列表 || [],
      }
    })
    ...
    for (let i = 0; i < 所有行.length; i++) {
      ...
      // N=0 是基準行，永遠顯示；其他行只有在有新增簡碼字時才顯示
      let 應顯示 =
        當前行.最有效率的簡碼個數 === 0 || 有新知乎 || 有新北語 || 有新臺標 || 有新古籍 || 有新繁簡

      // 額外檢查：如果該行所有列的值都與上一行相同，則不顯示
      if (應顯示 && i > 0) {
        const 前一行 = 所有行[i - 1]!
        const 所有列相同 =
  ...
  const 有省略行 = useMemo(() => {
    ...
    const 原始行數 = 分析結果.知乎簡體字頻結果.length
