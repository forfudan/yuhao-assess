  ...
  const 表格數據 = useMemo<表格行數據[]>(() => {
    ...
    const 所有行 = N值列表.map(N => {
      ...
      return {
        ...
        // 將字符串轉換爲字符數組（正確處理 Unicode 多位元組字符）
        知乎簡體字頻字符: Array.from(知乎結果?.對應字符字符串 || ''),
        北語簡體字頻字符: Array.from(北語結果?.對應字符字符串 || ''),
        臺標繁體字頻字符: Array.from(臺標結果?.對應字符字符串 || ''),
        古籍繁體字頻字符: Array.from(古籍結果?.對應字符字符串 || ''),
        繁簡聯合字頻字符: Array.from(繁簡結果?.對應字符字符串 || ''),
      }
    })
