...
const StaticDuplicateAnalysisPage: React.FC = () => {
  ...
  const 渲染表格 = () => {
    ...
    // 字集顯示順序（與 分析結果 的鍵名一一對應）
    const 字集顯示順序: 累積漢字集名稱型别[] = [
      'GB2312',
      '通用規範',
      '常用國字',
      'CJK基本',
    ...
    const 靜態重碼數據: 表格數據項[] = 字集顯示順序.map((字集名稱, 索引) => {
      ...
      return {
        key: String(索引 + 1),
        字集名稱,
        全碼重碼組數: 字集數據.全碼重碼組數.toLocaleString(),
        簡碼重碼組數: 字集數據.簡碼重碼組數.toLocaleString(),
        全碼重碼字數: 字集數據.全碼重碼字數.toLocaleString(),
        簡碼重碼字數: 字集數據.簡碼重碼字數.toLocaleString(),
        全碼選重個數: 全碼選重個數.toLocaleString(),
        簡碼選重個數: 簡碼選重個數.toLocaleString(),
        實際字符數: 字集數據.實際字符數.toLocaleString(),
        理論字符數: 字集數據.理論字符數.toLocaleString(),
        字集覆蓋率: (字集數據.字集覆蓋率 * 100).toFixed(2) + '%',
      }
    })
