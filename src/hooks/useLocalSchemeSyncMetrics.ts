 ...
 * 自動將最新內容（含方案名等配置）寫回 localStorage 中對應的本地方案條目。
 * 應在 MainLayout 中調用一次。
 */
...
export function useLocalSchemeSyncMetrics() {
  ...
  useEffect(() => {
    if (!當前標識符 || !當前方案) return
    // 確保當前方案的標識符與激活的本地方案標識符一致，否則跳過
    if (當前方案.元數據.標識符 !== 當前標識符) return
  ...
  }, [當前標識符, 當前方案, 動態選重, 靜態重碼, 候選個數, 速度當量, 簡碼效率, 鍵位熱力])
}
