...
/** 分佈的存檔格點間距，與 蒙特卡洛連續文本當量 的三位小數歸併一致 */
const 存檔步長 = 0.001

/**
 * 分佈的緊湊存檔形式
 *
 * 分佈本身是等距格點上的計數，用 `[{當量值, 個數}]` 存進方案 JSON
 * 光是重複的鍵名就佔掉四分之三的體積。存成「起點 + 個數序列」後
 * 同樣的信息只要約四分之一的空間，空格點記 0。
 */
export interface 緊湊分佈介面 {
  /** 第一個格點的當量值 */
  起始: number
  /** 格點間距 */
  步長: number
 ...
 * 把分佈壓成緊湊存檔形式
 */
export function 壓縮分佈(分佈: 分佈項介面[]): 緊湊分佈介面 {
  if (分佈.length === 0) return { 起始: 0, 步長: 存檔步長, 個數: [] }

  const 起始 = (分佈[0] as 分佈項介面).當量值
  const 末項 = 分佈[分佈.length - 1] as 分佈項介面
  const 格點數 = Math.round((末項.當量值 - 起始) / 存檔步長) + 1

  const 個數 = new Array<number>(格點數).fill(0)
  for (const { 當量值, 個數: 計數 } of 分佈) {
    const 下標 = Math.round((當量值 - 起始) / 存檔步長)
    if (下標 >= 0 && 下標 < 格點數) 個數[下標] = (個數[下標] as number) + 計數
  }
  return { 起始, 步長: 存檔步長, 個數 }
}

/**
 * 把緊湊存檔形式還原成分佈；空格點直接丟棄，與壓縮前等價
 */
export function 展開分佈(緊湊: 緊湊分佈介面): 分佈項介面[] {
  const 分佈: 分佈項介面[] = []
  緊湊.個數.forEach((個數, i) => {
    if (個數 <= 0) return
    // 浮點累加會漂移，用乘法並回到格點精度
    const 當量值 = Math.round((緊湊.起始 + i * 緊湊.步長) / 緊湊.步長) * 緊湊.步長
    分佈.push({ 當量值: Number(當量值.toFixed(6)), 個數 })
  })
  return 分佈
}
