 ...
 * @property 對應字符字符串 - 這 N 個使用簡碼的字符串，按效率排序（導出 JSON 時更簡潔）
 */
interface 簡碼效率結果介面 {
  最有效率的簡碼個數: number
  簡碼效率值: number
 ...
 *   [0, 5, 10, 25, 50, 100, ...]
 * )
 * // 結果[0] = { 最有效率的簡碼個數: 0, 簡碼效率值: 4.2, 對應字符字符串: '' }  // 全部用全碼
 * // 結果[5] = { 最有效率的簡碼個數: 100, 簡碼效率值: 3.1, 對應字符字符串: '的一...' }  // 前100個用簡碼
 */
export function 計算指定字頻下之簡碼效率(
  ...
  for (const N of N值列表) {
    ...
    結果.push({
      最有效率的簡碼個數: N,
      簡碼效率值,
      對應字符字符串: 選中字符列表.join(''), // 將數組連接成字符串，方便 JSON 導出
    })
  }
